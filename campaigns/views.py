...
def student_as_pdf(request, campaign_id, student_id):
    if request.user.is_authenticated():
        ...
        p.drawString(425, 820, 'До Директора на')
        p.drawString(425, 805, 'на ТУЕС')
        p.drawString(425, 790, 'към ТУ-София')
        p.setFont('Arial', 20)
        ...
        p.drawString(30, 715, 'живущ(настоящ адрес): %s' % (student.address))
        p.drawString(30, 685, 'Г-жо Директор,')
        ...
        p.drawString(30, 605, '1. Копие на медицинското свидетелство с отразени цветоусещане и диоптри')
